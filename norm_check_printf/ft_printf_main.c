...
/*   Updated: 2021/05/17 14:37:07 by jbyeon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					print_type(va_list cur_list, t_info *str_info)
{
	...
	if (str_info->type == 'c')
		...
		ret = print_num(va_arg(cur_list, unsigned int),
...
{
	if (c == '0' && str_info->width == 0 && str_info->prec == -1)
		str_info->zero = 1;
	else if (c == '-')
		str_info->minus = 1;
	else if (c == '.')
...
int					parsing2(char *str, va_list cur_list, t_info *str_info)
{
	int ret;
	...
	while (*str)
	{
		if (*str == '%')
		{
			...
			str_info->type = *str++;
			if ((str_info->minus == 1 || str_info->prec > -1)
					&& str_info->type != '%')
				str_info->zero = 0;
...
int					parsing_sentance(char *str, va_list cur_list)
{
	...
	if (!(str_info = malloc(sizeof(t_info))))
		return (-1);
	ret = parsing2(str, cur_list, str_info);
	free(str_info);
	return (ret);
}
